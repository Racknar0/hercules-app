...
router.get('/api/qa-data', async (req, res) => {
    try {
...
router.get('/api/check-files', async (req, res) => {
    try {
        const { nombre, dol } = req.query;
        if (!nombre || !dol) {
...
router.get('/api/qa-status', async (req, res) => {
    try {
        const { nombre, dol } = req.query;
        let allDocs = await MasterService.getAllDocumentsFlat();
        if (nombre && dol) {
...
router.post('/api/run-qa', async (req, res) => {
