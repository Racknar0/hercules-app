...
export default function MedicalOrganizer() {
    ...
    const fetchPendientes = (nombre, dol) => {
        ...
        fetch(
            `${API_BASE}/api/pendientes?nombre=${encodeURIComponent(nombre)}&dol=${encodeURIComponent(dol)}`,
        )
            .then((res) => res.json())
            .then((data) => {
    ...
    const fetchLoteDocuments = (nombre, dol) => {
        fetch(
            `${API_BASE}/api/lote-documents?nombre=${encodeURIComponent(nombre)}&dol=${encodeURIComponent(dol)}`,
        )
            .then((res) => res.json())
            .then((data) => {
    ...
    useEffect(() => {
        if (selectedLote) {
            ...
            fetch(
                `${API_BASE}/api/thinking?nombre=${encodeURIComponent(parsed.nombre)}&dol=${encodeURIComponent(parsed.dol)}`,
            )
                .then((res) => res.json())
                .then((data) => {
    ...
    const handleDeletePendiente = async (idx) => {
        ...
        try {
            const res = await fetch(
                `${API_BASE}/api/pendiente?index=${idx}&nombre=${encodeURIComponent(parsed.nombre)}&dol=${encodeURIComponent(parsed.dol)}`,
                { method: 'DELETE' },
            );
            const data = await res.json();
            if (data.success) fetchAll();
    ...
    const handleDeleteRecord = async (archivoOrigen) => {
        ...
        try {
            const res = await fetch(
                `${API_BASE}/api/records?archivoOrigen=${encodeURIComponent(archivoOrigen)}&nombre=${encodeURIComponent(parsed.nombre)}&dol=${encodeURIComponent(parsed.dol)}`,
                { method: 'DELETE' },
            );
            const data = await res.json();
            if (data.success) fetchAll();
                ...
                <>
                    {files.length > 0 && (
                        ...
                        >
                            ...
                            >
                                🛡️ Perfilado Oficial de Carpeta (Blindaje IA)
                            </h3>
                            <div
                                style={{
                                    display: 'flex',
                                                                ...
                                                                <tr key={`mp-${providerName}-${iIdx}`}>
                                                                    {isFirstForDoc && (
                                                                        <td rowSpan={docRowSpan}>
                                                                            ...
                                                                            <br />
                                                                            <button className="btn-sm btn-reject" onClick={() => handleDeleteRecord(doc.archivoOrigen)} style={{marginTop: '5px', padding: '2px 8px', fontSize: '0.7rem'}}>🗑️ Eliminar</button>
                                                                            ...
                                                                            {pageRescanTarget === doc.archivoOrigen && (
                                                                                <div style={{marginTop: '4px', display: 'flex', gap: '3px', alignItems: 'center'}}>
                                                                                    ...
                                                                                    <button onClick={() => { if (pageRescanInput.trim()) handleRescanDoc(doc.archivoOrigen, pageRescanInput.trim()); }} disabled={!pageRescanInput.trim()} style={{padding: '3px 6px', fontSize: '0.65rem', borderRadius: '4px', background: 'rgba(138,43,226,0.5)', color: 'white', border: 'none', cursor: 'pointer'}}>▶</button>
                                                                                </div>
                                                                            )}
                                                                        </td>
                                                                    )}
